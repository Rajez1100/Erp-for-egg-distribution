...
export class RedTableComponent implements OnInit {
    ...
    private classNamesList: Object = {
        ...
        "Change Isv Status":{name:'Change Status',iconClass:"fa fa-clock-o",mainClass:"btn-primary"},
    ...
    public ngOnInit(): void {
        ...
        this.subscriptions['routerEventSubscription'] = this.router.events
            ...
            .subscribe((event: NavigationEnd) => {
                ...
                if (event.urlAfterRedirects == localStorageData.url) {
    ...
    public getFilterValue(type) {
        if (this.columnFilter.length) {
            for (let i = 0; i < this.columnFilter.length; i++) {
                this.filterValues.forEach((value: any, index) => {
                    if (this.columnFilter[i]['name'] == this.filterValues[index]['name']) {
                        this.filterValues[index]['value'] = this.columnFilter[i]['value'];
                    } else {
                        this.canAdd = this.canAddFilter(this.filterValues, this.columnFilter[i]['name']);
                        if (this.canAdd == true) {
                            this.filterValues.push({ 'name': this.columnFilter[i]['name'], 'value': this.columnFilter[i]['value'] })
                        }
                    }
                });
            }
