...
export class UserService {
    ...
    getUsers(): Observable<User[]> {
        ...
        return this.http.get(GetApiurl('users/'), { headers: headers })
            .map(Response => {
                return Response.json().map(u => new User(u));
            })
    ...
    addUser(u: User): Observable<User> {
        ...
        return this.http.post(GetApiurl('users/'), JSON.stringify(u), { headers: headers })
            .map(this.extractUser)
            .catch(this.handleError);
    }
    ...
    updateUser(u: User): Observable<User> {
        ...
        headers.append('Authorization', 'JWT ' + localStorage.getItem('id_token'));
        headers.append('Content-Type', 'application/json')
        return this.http.put(GetApiurl('users/') + u.id + '/', JSON.stringify(u), { headers: headers })
            .map(this.extractUser)
            .catch(this.handleError);
    }
